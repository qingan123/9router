#!/usr/bin/env bash
set -Eeuo pipefail
APP_DIR=${APP_DIR:-/opt/9router}; PORT=${PORT:-20128}; REPO_URL=${REPO_URL:-https://github.com/qingan123/9router.git}
...
APP_DIR=$(read_tty "目录 [$APP_DIR]: "); APP_DIR=${APP_DIR:-/opt/9router}; PORT=$(read_tty "端口 [$PORT]: "); PORT=${PORT:-20128}; [[ $PORT =~ ^[0-9]+$ ]] || fail '端口无效'
